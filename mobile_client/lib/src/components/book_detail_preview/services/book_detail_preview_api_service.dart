import 'dart:convert';
import 'dart:io';

import 'package:http/http.dart' as http;
import 'package:mobile_client/src/core/config/app_config.dart';
import 'package:mobile_client/src/home/models/api_response_generic.dart';
import 'package:mobile_client/src/home/models/book_response.dart';

class BookDetailPreviewApiService {
  static const String defaultBaseUrl = AppConfig.apiBaseUrl;

  BookDetailPreviewApiService({
    required this.baseUrl,
    http.Client? client,
  }) : _client = client ?? http.Client();

  final String baseUrl;
  final http.Client _client;

  Map<String, String> get _headers => {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
      };

  Future<ApiResponseGeneric<BookResponse>> getBookDetails({
    required String token,
    required int id,
  }) async {
    final response = await _guardedRequest(
      () => _client.get(
        Uri.parse('$baseUrl/books/$id'),
        headers: {
          ..._headers,
          'Authorization': 'Bearer $token',
        },
      ),
    );

    final body = _decodeJson(response.body);
    _ensureSuccess(response.statusCode, body);

    final apiResponse = ApiResponseGeneric<BookResponse>.fromJson(
      body,
      (json) => BookResponse.fromJson(json),
    );

    return apiResponse;
  }

  Map<String, dynamic> _decodeJson(String rawBody) {
    if (rawBody.trim().isEmpty) {
      return <String, dynamic>{};
    }
    final dynamic decoded = jsonDecode(rawBody);
    if (decoded is Map<String, dynamic>) {
      return decoded;
    }
    throw BookDetailPreviewApiException('Response format không hợp lệ.');
  }

  void _ensureSuccess(int statusCode, Map<String, dynamic> body) {
    if (statusCode < 200 || statusCode >= 300) {
      throw BookDetailPreviewApiException(_extractErrorMessage(body, statusCode));
    }

    final code = _extractCode(body);

    if (code != 1000) {
      final msg = _extractErrorMessage(body, statusCode);
      throw BookDetailPreviewApiException(msg);
    }
  }

  int _extractCode(Map<String, dynamic> body) {
    final dynamic value = body['code'];
    if (value is int) {
      return value;
    }
    if (value is String) {
      return int.tryParse(value) ?? 1000;
    }
    return 1000;
  }

  String _extractErrorMessage(Map<String, dynamic> body, int statusCode) {
    return body['message']?.toString() ??
        body['error']?.toString() ??
        'Request thất bại ($statusCode).';
  }

  Future<http.Response> _guardedRequest(
    Future<http.Response> Function() request,
  ) async {
    try {
      return await request();
    } on SocketException {
      throw BookDetailPreviewApiException(
        'Không thể kết nối máy chủ. Kiểm tra API đang chạy và base URL.',
      );
    } on http.ClientException catch (error) {
      throw BookDetailPreviewApiException('Lỗi kết nối: ${error.message}');
    }
  }
}

class BookDetailPreviewApiException implements Exception {
  const BookDetailPreviewApiException(this.message);

  final String message;

  @override
  String toString() => message;
}
